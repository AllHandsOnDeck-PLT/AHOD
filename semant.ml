...
let check (globals, action_decls, main_decl) = (*changed from class_decls to action_decls*)
  ...
  let check_action act = 
    ignore(check_binds "aparams" act.aparams);
    ignore(check_binds "alocals" act.alocals);

    let symbols = List.fold_left (fun m (ty, name) -> StringMap.add name ty m)
        StringMap.empty (globals @act.aparams @ act.alocals ) in 
...
in (globals, List.map check_action action_decls, check_action main_decl)